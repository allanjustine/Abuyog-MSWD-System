        ...
        <div class="form-group col-md-4" hidden>
            ...
            <select name="educational_attainment" id="educational_attainment" class="form-control" required>
                ...
                <option value="No Formal Education"
                    {{ old('educational_attainment', Auth::user()?->basicInfo?->educational_attainment) === 'No Formal Education' ? 'selected' : '' }}>
                    None
                </option>
                <option value="Kindergarten"
                    {{ old('educational_attainment', Auth::user()?->basicInfo?->educational_attainment) === 'Kindergarten' ? 'selected' : '' }}>
                    Kindergarten
                </option>
                <option value="Elementary"
                    {{ old('educational_attainment', Auth::user()?->basicInfo?->educational_attainment) === 'Elementary' ? 'selected' : '' }}>
                    Elementary
                </option>
                <option value="High School"
                    {{ old('educational_attainment', Auth::user()?->basicInfo?->educational_attainment) === 'High School' ? 'selected' : '' }}>
                    Junior High School
                </option>
                <option value="Senior High"
                    {{ old('educational_attainment', Auth::user()?->basicInfo?->educational_attainment) === 'Senior High' ? 'selected' : '' }}>
                    Senior High School
                </option>
                <option value="College"
                    {{ old('educational_attainment', Auth::user()?->basicInfo?->educational_attainment) === 'College' ? 'selected' : '' }}>
                    College
                </option>
                <option value="Vocational"
                    {{ old('educational_attainment', Auth::user()?->basicInfo?->educational_attainment) === 'Vocational' ? 'selected' : '' }}>
                    Vocational
                </option>
                <option value="Post Graduate"
                    {{ old('educational_attainment', Auth::user()?->basicInfo?->educational_attainment) === 'Post Graduate' ? 'selected' : '' }}>
                    Post Graduate
                </option>
            </select>
        </div>
    ...
    <div class="form-row">
        ...
        <div class="form-group col-md-6">
            ...
            <input type="text" name="other_skills" class="form-control" placeholder="Other Skills"
                value="{{ old('other_skills', Auth::user()?->basicInfo?->skills ?? '') }}"
...
<script>
    document.addEventListener('DOMContentLoaded', function() {
        ...
        const hasMinorChild = @json(Auth::user()?->has_minor_child);
        const age = @json(Auth::user()?->age);
        ...
        console.log(age, hasMinorChild)
        submitButton.addEventListener('click', function(e) {
            if (age < 60 || isNaN(age) && !hasMinorChild) {
                e.preventDefault(); // Prevent form submission
                Swal.fire({
                    icon: 'error',
                    title: 'Invalid Submission',
                    text: 'Please ensure the applicant is 60 years old or older or has minor child.',
                    confirmButtonText: 'OK'
                });
            }
        });
